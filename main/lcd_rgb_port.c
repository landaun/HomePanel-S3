...
void gpio_init(void)
{
    ...
    io_conf.pin_bit_mask = TOUCH_RST_GPIO_SEL;
    // Set as output mode
...
esp_err_t lcd_rgb_init()
{
    ESP_LOGI(
        TAG,
        "Install RGB LCD panel driver"); // Log the start of the RGB LCD panel driver installation
    ...
    ESP_LOGI(TAG, "Enable LCD panel output");
    esp_err_t err = esp_lcd_panel_disp_on_off(panel_handle, true);
    if (err == ESP_ERR_NOT_SUPPORTED)
    {
        ESP_LOGW(TAG, "Panel driver does not support disp_on_off, skipping");
    }
    else
    {
        ESP_ERROR_CHECK(err);
    }
    ESP_ERROR_CHECK(lcd_rgb_bl_on());
    ...
    ESP_ERROR_CHECK(esp_lcd_touch_new_i2c_gt911(
        tp_io_handle, &tp_cfg, &tp_handle)); // Create new I2C GT911 touch controller
#endif                                       // CONFIG_EXAMPLE_LCD_TOUCH_CONTROLLER_GT911

    ESP_ERROR_CHECK(lvgl_port_init(panel_handle,
