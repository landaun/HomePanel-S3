...
/*
 * The RGB panel header and frame buffer function were renamed in ESP-IDF
 * v5.0. Guard the version macros so we fall back to the old header when
 * building against releases that don't provide them.
 */
...
#elif __has_include("esp_lcd/rgb/include/esp_lcd_panel_rgb.h")
#include "esp_lcd/rgb/include/esp_lcd_panel_rgb.h"
#else
#error "esp_lcd_panel_rgb header not found; ensure RGB LCD support is enabled in ESP-IDF"
#endif
#else
#include "esp_lcd_panel_rgb.h"
#endif
...
#endif
#define ESP_LCD_RGB_GET_FRAME_BUFFER esp_lcd_rgb_panel_get_frame_buffer
...
static void* get_next_frame_buffer(esp_lcd_panel_handle_t panel_handle)
{
    ...
    if (next_fb == NULL)
    {
        ESP_ERROR_CHECK(ESP_LCD_RGB_GET_FRAME_BUFFER(panel_handle, 2, &fb[0],
                                                     &fb[1])); // Get the frame buffers
...
IRAM_ATTR static void rotate_copy_pixel(const uint16_t* from, uint16_t* to, uint16_t x_start,
...
static lv_port_flush_probe_t flush_copy_probe(lv_disp_drv_t* drv)
{
    ...
    for (int i = 0; i < disp_refr->inv_p; i++)
    {
        ...
        {
            flush_ver = (disp_refr->inv_areas[i].y2 + 1 -
                         disp_refr->inv_areas[i].y1); // Calculate vertical flush size
            flush_hor = (disp_refr->inv_areas[i].x2 + 1 -
...
static void flush_dirty_copy(void* dst, void* src, lv_port_dirty_area_t* dirty_area)
{
    ...
    for (int i = 0; i < dirty_area->inv_p; i++)
    {
        ...
        if (dirty_area->inv_area_joined[i] == 0)
        {
            ...
            rotate_copy_pixel(src, dst, x_start, y_start, x_end, y_end, LV_HOR_RES, LV_VER_RES,
...
static void flush_callback(lv_disp_drv_t* drv, const lv_area_t* area, lv_color_t* color_map)
{
    ...
    if (lv_disp_flush_is_last(drv))
    {
        ...
        if (drv->full_refresh)
        {
            ...
            rotate_copy_pixel((uint16_t*)color_map, next_fb, offsetx1, offsety1, offsetx2, offsety2,
...
static lv_disp_t* display_init(esp_lcd_panel_handle_t panel_handle)
{
    ...
    static lv_disp_draw_buf_t disp_buf = {
        0}; // Contains internal graphic buffer(s) called draw buffer(s)
...
#if LVGL_PORT_AVOID_TEAR_ENABLE
    // To avoid tearing effect, at least two frame buffers are needed: one for LVGL rendering and
    // another for RGB output
    buffer_size = LVGL_PORT_H_RES * LVGL_PORT_V_RES;
#if (LVGL_PORT_LCD_RGB_BUFFER_NUMS == 3) && (EXAMPLE_LVGL_PORT_ROTATION_DEGREE == 0) &&            \
    LVGL_PORT_FULL_REFRESH
    // With three buffers and full-refresh, one buffer is always available for rendering
    ESP_ERROR_CHECK(
        ESP_LCD_RGB_GET_FRAME_BUFFER(panel_handle, 3, &lvgl_port_rgb_last_buf, &buf1, &buf2));
    ...
    // Using three frame buffers, one for LVGL rendering and two for RGB driver (one used for
    // rotation)
    void* fbs[3];
    ESP_ERROR_CHECK(ESP_LCD_RGB_GET_FRAME_BUFFER(panel_handle, 3, &fbs[0], &fbs[1], &fbs[2]));
    buf1 = fbs[2]; // Set buf1 to the third frame buffer
#else
    ESP_ERROR_CHECK(
        ESP_LCD_RGB_GET_FRAME_BUFFER(panel_handle, 2, &buf1, &buf2)); // Get two frame buffers
#endif
#else
    ...
    buf1 = heap_caps_malloc(buffer_size * sizeof(lv_color_t),
                            LVGL_PORT_BUFFER_MALLOC_CAPS); // Allocate memory
    assert(buf1);                                          // Ensure allocation succeeded
    ESP_LOGI(TAG, "LVGL buffer size: %dKB",
             buffer_size * sizeof(lv_color_t) / 1024); // Log buffer size
#endif /* LVGL_PORT_AVOID_TEAR_ENABLE */
...
static void touchpad_read(lv_indev_drv_t* indev_drv, lv_indev_data_t* data)
{
    ...
    bool touchpad_pressed = esp_lcd_touch_get_coordinates(
        tp, &touchpad_x, &touchpad_y, NULL, &touchpad_cnt, 1); // Get touch coordinates
    ...
    // Notify screen manager of touch activity for timeout/wake handling
    screen_manager_touch_activity(data->state == LV_INDEV_STATE_PRESSED);
}
...
static esp_err_t tick_init(void)
{
    ...
    return esp_timer_start_periodic(lvgl_tick_timer,
                                    LVGL_PORT_TICK_PERIOD_MS * 1000); // Start the timer
}
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle)
{
    ...
    BaseType_t ret = xTaskCreatePinnedToCore(lvgl_port_task, "lvgl", LVGL_PORT_TASK_STACK_SIZE,
                                             NULL, LVGL_PORT_TASK_PRIORITY, &lvgl_task_handle,
                                             core_id); // Create the LVGL task
    if (ret != pdPASS)
    {
