...
static const char* const s_nvs_keys[DISC_DOMAIN_COUNT] = {
    [DISC_DOMAIN_LIGHT]       = NVS_KEY_LIGHTS,
    [DISC_DOMAIN_TEMPERATURE] = NVS_KEY_TEMPS,
    [DISC_DOMAIN_CLIMATE]     = NVS_KEY_CLIMATE,
    [DISC_DOMAIN_OCCUPANCY]   = NVS_KEY_OCC,
    [DISC_DOMAIN_SWITCH]      = NVS_KEY_SWITCH,
    [DISC_DOMAIN_MEDIA]       = NVS_KEY_MEDIA,
    [DISC_DOMAIN_SCENE]       = NVS_KEY_SCENES,
    [DISC_DOMAIN_AUTOMATION]  = NVS_KEY_AUTO,
};

// Human-readable labels for each domain (used as tab titles)
static const char* const s_domain_labels[DISC_DOMAIN_COUNT] = {
    [DISC_DOMAIN_LIGHT]       = "Lights",
    [DISC_DOMAIN_TEMPERATURE] = "Temperature",
    [DISC_DOMAIN_CLIMATE]     = "Climate",
    [DISC_DOMAIN_OCCUPANCY]   = "Occupancy",
    [DISC_DOMAIN_SWITCH]      = "Switches",
    [DISC_DOMAIN_MEDIA]       = "Media",
    [DISC_DOMAIN_SCENE]       = "Scenes",
                                     ...
                                     const char* device_class)
{
    ...
    if (starts_with(entity_id, "sensor."))
    {
        ...
        if (unit && (strcmp(unit, "\xC2\xB0""F") == 0 || strcmp(unit, "\xC2\xB0""C") == 0 ||
                     strcmp(unit, "°F") == 0 || strcmp(unit, "°C") == 0 ||
                     strcmp(unit, "F") == 0  || strcmp(unit, "C") == 0))
            return DISC_DOMAIN_TEMPERATURE;
...
static esp_err_t build_area_ctx(const char* area_name, disc_area_ctx_t* ctx)
{
    ...
    cJSON_ArrayForEach(area_entry, area_list)
    {
        ...
        if (name && cJSON_IsString(name) && id && cJSON_IsString(id))
        {
            ...
            {
                snprintf(ctx->target_area_id, sizeof(ctx->target_area_id), "%s",
                         id->valuestring);
                break;
            }
        }
...
esp_err_t discovery_run(const char* area_filter, disc_result_t* out_result)
{
    ...
    out_result->entities = heap_caps_calloc(total_count, sizeof(disc_entity_t),
...
static size_t serialize_domain(const disc_result_t* result, disc_domain_t domain,
    ...
    for (size_t i = 0; i < result->count; i++)
    {
        ...
        switch (domain)
        {
            ...
            case DISC_DOMAIN_OCCUPANCY:
                n = snprintf(line, sizeof(line), "%s,%s,%s\n",
                             e->entity_id, e->friendly_name, e->device_class);
                break;
            case DISC_DOMAIN_AUTOMATION:
                n = snprintf(line, sizeof(line), "%s,%s,%d\n",
                             e->entity_id, e->friendly_name, e->enabled ? 1 : 0);
                break;
            default:
...
esp_err_t discovery_load_cache(disc_result_t* out_result)
{
    ...
    out_result->entities = heap_caps_calloc(cached_count, sizeof(disc_entity_t),
                                            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!out_result->entities)
