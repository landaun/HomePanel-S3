...
/*
 * The RGB panel header was renamed in ESP-IDF v5.0 from esp_lcd_rgb_panel.h
 * to esp_lcd_panel_rgb.h. Guard the version macros so that older releases that
 * don't define them default to the original header.
 */
...
#else
#error "esp_lcd_panel_rgb header not found; ensure RGB LCD support is enabled"
#endif
#else
#include "esp_lcd_panel_rgb.h"
#endif
...
#define I2C_MASTER_SCL_IO CONFIG_EXAMPLE_I2C_SCL_IO /*!< GPIO number used for I2C master clock */
#define I2C_MASTER_SDA_IO CONFIG_EXAMPLE_I2C_SDA_IO /*!< GPIO number used for I2C master data  */
...
#define EXAMPLE_PIN_NUM_TOUCH_RST CONFIG_EXAMPLE_PIN_NUM_TOUCH_RST // -1 if not used
#define EXAMPLE_PIN_NUM_TOUCH_INT CONFIG_EXAMPLE_PIN_NUM_TOUCH_INT // -1 if not used

bool example_lvgl_lock(int timeout_ms);
void example_lvgl_unlock(void);

esp_err_t lcd_rgb_init();

esp_err_t lcd_rgb_bl_on();
esp_err_t lcd_rgb_bl_off();
